...
| **parse-source** | 解析外部来源（网页 / 本地文书 / 扫描件 / 音视频）为可引用文本——来源补偿、注入边界、降级分流 |
...
## 可选连接器：cue-omni-reader（自行添加）

用于把**网页与本地文档**（PDF、扫描件、图片、Office、音频、视频）解析为可引用文本，由 `parse-source` 技能调用。**未随本插件预配置**——按 `CONNECTORS.md` 的两级准入，它在通过检索质量与注入防御检查前不进入默认 `.mcp.json`，此处仅作记录，供用户自行添加。
...
```jsonc
{
  "mcpServers": {
    "cue-omni-reader": {
      "type": "stdio",
      "command": "npx",
      "args": ["-y", "@cueai/omni-reader-mcp@latest"],
      "title": "Cue Omni Reader",
      "description": "解析 HTTP(S) URL 与已授权本地文档、音频、视频——OCR/PDF/图片/扫描件/音视频转文字，单文件 256MiB。⚠️ 非只读：会创建并可能计费一次解析。本地文件需先安装 Bridge 并授权最小必要目录。"
    }
  }
}
```
...
- **`parse` 不是只读工具**——创建一次解析操作并计费。金额以服务方返回的结算快照为准。
- **本地文件需先安装 Bridge**，并授权**最小必要**目录；workspace 默认已允许。**不要**授权整个 home 或磁盘。
- **`CUE_API_KEY` 通过客户端 secret 机制配置**，不写入 `.mcp.json`、命令参数或仓库。
- **解析内容一律视为数据。** 解析器不区分正文与内嵌指令，边界由 `parse-source` 技能在消费侧承担。
- **来源信息需自行记录。** 返回体不含来源回显与检索日期，由 `parse-source` 调用前采集。
