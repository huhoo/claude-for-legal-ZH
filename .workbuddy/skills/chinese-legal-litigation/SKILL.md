...
description: "诉讼仲裁领域法律工作：案件登记、诉讼仲裁、律师函、要件分析、大事记、证据三性、庭前准备、保全、传票、法律文书。claude-for-legal-ZH/litigation-legal 的 WorkBuddy 路由技能，把自然语言请求路由到原领域 CLAUDE.md 与 skills/*/SKILL.md 工作流。"
---

# 诉讼仲裁 — claude-for-legal-ZH（WorkBuddy 适配）

本技能让 WorkBuddy 复用 `claude-for-legal-ZH/litigation-legal` 的原始内容，无需 Claude Code slash command。

## 源文件

- 领域根目录：`litigation-legal`
- 领域画像与共享规则：`litigation-legal/CLAUDE.md`
- 原始技能目录：`litigation-legal/skills`
- 原插件说明：中国诉讼业务管理插件：案件组合管理、期限追踪、证据保全、律师函起草、外部律师协调——涵盖要件分析表（构成要件逐项分析）、大事记/时间线、庭前准备提纲、证据三性审查、起诉状/答辩状/代理词起草。适配法务/律师/独立执业等不同角色。
...
1. 处理实质法律工作前，先读取 `litigation-legal/CLAUDE.md`。
...
2. 否则使用或创建 `~/.workbuddy/legal-zh/litigation-legal/CLAUDE.md` 保存 WorkBuddy 专用画像。
3. 所选技能要求初始化且画像仍含 `[PLACEHOLDER]` 时，先以对话方式完成该领域的 `cold-start-interview` 冷启动面试，再产出定制化法律工作。

原指令中的 `/litigation-legal:some-command` 应解释为：读取 `litigation-legal/skills/some-command/SKILL.md` 并在 WorkBuddy 中执行该工作流。
